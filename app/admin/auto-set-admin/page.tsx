...
export default async function AutoSetAdminPage() {
  ...
  try {
    ...
    // Now insert the admin role directly using the correct table structure
    // Based on the database.types.ts, the user_roles table has: id, name, description, created_at
    // It does NOT have a user_id column
    const { error: insertError } = await supabase.from("user_roles").insert({
      name: "admin",
      description: `Administrator for ${email}`,
  ...
  return (
    <div className="container mx-auto py-10">
      <Card className="max-w-md mx-auto">
        <CardHeader>
          <CardTitle>Automatic Admin Setup</CardTitle>
          <CardDescription>Setting {email} as an admin user</CardDescription>
        </CardHeader>
        <CardContent>
          <Alert className={result.success ? "bg-blue-900 text-white" : "bg-blue-900 text-white"}>
            {result.success ? (
                ...
                <div className="space-y-2">
                  ...
                  {tableInfo && (
                    <details className="mt-2 text-xs">
                      ...
                      <pre className="mt-2 p-2 bg-gray-100 rounded overflow-x-auto text-black">
