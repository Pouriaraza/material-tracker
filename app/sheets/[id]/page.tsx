...
export default function SheetPage({ params }: { params: { id: string } }) {
  ...
  const addNewRow = async () => {
    ...
    try {
      // Use the new API endpoint for safe row creation
      const response = await fetch(`/api/sheets-new/${params.id}/rows`, {
        method: "POST",
        headers: {
  ...
  const saveChanges = async () => {
    ...
    try {
      ...
      for (let i = 0; i < changesToSave.length; i += batchSize) {
        ...
        await Promise.all(
          batch.map(async (change) => {
            ...
            // Use the new API endpoint for cell updates
            const response = await fetch(`/api/sheets-new/${params.id}/rows`, {
              method: "PATCH",
              headers: {
                "Content-Type": "application/json",
      ...
      toast({
        ...
        description: err.message || "Some changes could not be saved. Please try again.",
        variant: "destructive",
      })
    } finally {
      setIsSaving(false)
  ...
  const deleteRow = async (rowId: string) => {
    ...
    try {
      // Use the new API endpoint for row deletion
      const response = await fetch(`/api/sheets-new/${params.id}/rows`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
      ...
      toast({
        title: "Error",
        description: err.message || "Failed to delete row",
        variant: "destructive",
      })
    }
  }
  ...
  const importFromExcel = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        await supabase.from("rows").update({ is_deleted: true }).eq("sheet_id", params.id)
    ...
    <main className="container mx-auto py-6 px-4 md:px-6">
      ...
      <div className="flex flex-wrap gap-4 mb-6 justify-between items-center">
        <div className="flex gap-2 flex-wrap">
          ...
          <Button onClick={addNewRow} variant="outline" className="flex items-center gap-2 bg-transparent">
            <Plus className="h-4 w-4" />
            New Row
          </Button>
          <Button onClick={addStatusColumn} variant="outline" className="flex items-center gap-2 bg-transparent">
